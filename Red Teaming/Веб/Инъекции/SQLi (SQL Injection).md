...
- SQLi может находиться в следующих местах:
	- Пользовательские формы ввода (POST-запросы)
	- Параметры запросов (GET-запросы): `https://<url>/filter?Category=<param>`->`SELECT * FROM product WHERE category='<param>')`
	- HTTP-заголовки
	- Куки: `TrackId=<cookie>`-> `SELECT trackId FROM ids where trackId='<cookie>'`
	- JSON, XML: приложения могут получать информацию в виде JSON, XML, которая далее используется для SQL-запросов. Атакующий может вставить SQLi в JSON, XML и использовать разные кодировки для обхода защиты ([hex/dec-кодировки](https://www.howtocreate.co.uk/sidehtmlentity.html)): 
```xml
<stockCheck> 
...
#### Обход аутентификации с помощью UNION
Авторизация в приложении может работать следующим образом:
1. Сервер берет введенный логин и делает запрос:  `SELECT password FROM users WHERE username = '<логин>'`
2. Из БД возвращается хэш пароля
3. PHP-код берет пароль, который введен в форму, хэширует его и сравнивает с тем хэшем, который вернулся из базы данных.
Если в приложении присутствует SQLi в форме авторизации, можно с помощью `UNION SELECT` подставить хэш пароля, который атакующий знает. 
Действия атакующего:
1. Атакующий берет любой пароль
2. Хэширует егов том формате, который использует приложение
3. В поле `login` атакующий вводит пейлоад, который заставит базу вернуть именно этот хэш.
Пейлод: `fake_user' UNION SELECT '<fake_hash>'-- -`. Нужно использовать несуществующего пользователя, чтобы SQL-запрос вернул 0 строк и PHP-скрипт использовал подставленного пользователя.
#### DIOS (Dump in One Shot)
Техника, позволяющая извлечь всю нужную информацию из базы данных за один SQL-запрос. Пример дампа всех имен баз данных для MySQL: 
```SQL
1' UNION SELECT (select(@a)from(select(@a:=0x00),(select (@a) from (information_schema.schemata)where(@a)in(@a:=concat(@a, schema_name,'\n'))))a),2 -- -
```
### Blind
...
```
При использовании `AND SLEEP()` обязательно нужно указывать перед инъекцией значение, которое существует в БД. Иначе запрос до инъекции вернёт `false` и до выполнения `SLEEP()` запрос не дойдёт.
...
### Routed SQLi
Маршрутизируемая SQL-инъекция применяется, если результаты выполнения первого запроса к СУБД передаются во второй SQL запрос. Другими словами, можно сказать, что это инъекция в инъекции. Данную инъекцию имеет смысл пробовать эксплуатировать если UNION SQL-инъекция не дает результата, но прямых ошибок мы не получаем.
Пример: `http://example.com/index.php?id=1' union select "n'",2,3 -- -`
Должна появиться ошибка, говорящая о синтаксической ошибке в запросе SQL. С этого момента становится известно место, куда надо произвести еще одну инъекцию для получения результата.
### ORM
ORM (Object-Relational Mapping) - это сопоставление объектов с реляционной базой данных. Данная технология позволяет работать с данными в БД, используя ООП языка программирования. При использовании ORM разработчикам не нужно составлять SQL-запрос вручную, вместо этого они могут создать объект на используемом языке программирования и дальнейшее связывание объекта и базы данных будет осуществлять ORM. Использование ORM во многих случаях предотвращает SQL-инъекции, но как и любая технология она может иметь уязвимости. 
Информация по инъекциям ORM:
- [Эксплуатация инъекций в Hibernate ORM](https://habr.com/ru/companies/parallels/articles/272589/)
- [HQL Injection Exploitation in MySQL](https://www.levelblue.com/blogs/spiderlabs-blog/hql-injection-exploitation-in-mysql)
